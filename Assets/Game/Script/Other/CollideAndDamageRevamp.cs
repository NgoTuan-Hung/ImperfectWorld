...
public class CollideAndDamageRevamp : MonoEditor, IGameEffectBehaviour
{
    ...
    public void Enable(GameEffectSO p_gameEffectSO)
    {
        ...
        if (p_collideAndDamageSO.spawnEffectOnCollide)
        {
            ...
            if (!p_collideAndDamageSO.collisionEffectPoolType.Equals(EffectPool.LaterDecide))
                collisionEffectPool = GameManager.Instance.GetEffectPool(
                    p_collideAndDamageSO.collisionEffectPoolType
                );
