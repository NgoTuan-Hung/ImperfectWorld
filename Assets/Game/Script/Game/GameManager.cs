...
public class GameManager : MonoSingleton<GameManager>
{
	...
	private void Awake() 
	{
		...
		{
			spawnObjectPools.Add(new ObjectPool
			(
	...
	private void Start() 
	{
		for (int i=0;i<spawnEnemyInfos.Count;i++)
		{
			...
			spawnObjectPools[i].ForEach(poolObject => 
			{
				poolObject.customMono.stat.healthReachZeroEvent.action += () => t_spawnEnemyInfo.currentSpawn--;
			});
		}
	...
	IEnumerator HandleWave()
	{
		while (true)
		{
			...
			pickedSpawnEnemyIndex = spawnCumulativeDistribution.CumulativeDistributionBinarySearch(0, spawnCumulativeDistribution.Count-1, rand);
			...
			{
				CustomMono customMono = spawnObjectPools[pickedSpawnEnemyIndex].PickOne().customMono;
			...
			if (stopwatch.Elapsed.TotalSeconds > waveDuration) newWave = true;
		}
	}
	...
	void CalculateSpawnCumulativeDistribution()
	{
		...
		{
			spawnCumulativeDistribution[i] = spawnCumulativeDistribution[i-1] + spawnChances[i] / t_total;
		}
	}
	...
	public void AddCustomMono(CustomMono customMono)
	{
		...
		if (customMono.allyTags.Contains("GamePlayer")) playerAllies.Add(customMono);
	}
