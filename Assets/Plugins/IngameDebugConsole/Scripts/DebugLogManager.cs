...
#pragma warning disable 0649
        ...
        [Tooltip(
            "If enabled, clicking the resize button of the console window will copy all logs to clipboard. It'll also play a scale animation to give feedback."
        )]
...
#pragma warning restore 0414
        ...
        [SerializeField]
        [Tooltip(
            "If a log that isn't expanded is longer than this limit, it will be truncated. This greatly optimizes scrolling speed of collapsed logs if their log messages are long."
        )]
        internal int maxCollapsedLogLength = 200;

        [SerializeField, UnityEngine.Serialization.FormerlySerializedAs("maxLogLength")]
        [Tooltip(
            "If an expanded log is longer than this limit, it will be truncated. This optimizes scrolling speed while an expanded log is visible."
        )]
...
#pragma warning restore 0649
        ...
        public string GetAllLogs(int maxLogCount, float maxElapsedTime)
        {
            ...
            int startIndex =
                uncollapsedLogEntries.Count - Mathf.Min(uncollapsedLogEntries.Count, maxLogCount);
            if (uncollapsedLogEntriesTimestamps != null)
            {
                float currentElapsedSeconds = Time.realtimeSinceStartup;
                while (
                    startIndex < uncollapsedLogEntries.Count
                    && currentElapsedSeconds
                        - uncollapsedLogEntriesTimestamps[startIndex].elapsedSeconds
                        > maxElapsedTime
                )
                    startIndex++;
            }
